...
import com.mik1ng.network.environment.NetworkEnvironmentActivity;
import com.mik1ng.network.errorhandler.ExceptionHandle;
import com.mik1ng.network.errorhandler.HttpErrorHandler;
import com.mik1ng.network.interceptor.RequestInterceptor;
import com.mik1ng.network.interceptor.ResponseInterceptor;
...
public class NetworkApi {
    ...
    private static OkHttpClient getOkHttpClient() {
        ...
        if (okHttpClient == null) {
            ...
            builder.addInterceptor(new RequestInterceptor(mContext, iNetworkRequiredInfo));
        ...
        return new ObservableTransformer<T, T>() {
            ...
            public ObservableSource<T> apply(Observable<T> upstream) {
                Observable<T> observable = upstream
                        ...
                        .observeOn(AndroidSchedulers.mainThread())//观察Android主线程
                        .map(NetworkApi.<T>getAppErrorHandler())//判断有没有500的错误，有则进入getAppErrorHandler
                        .onErrorResumeNext(new HttpErrorHandler<T>(mContext));//判断有没有400的错误
                //这里还少了对异常
                //订阅观察者
//                observable.subscribe(observer);
                return observable;
